...
**elastix** is a prototype and case study project. Whether you're fixing a bug, improving the UI, or suggesting a new simulation logic, your help is welcome. We want to keep this project fun and educational.
...
3.  **Clone it**:
    ```bash
    git clone [https://github.com/YOUR-USERNAME/elastix.git](https://github.com/YOUR-USERNAME/elastix.git)
    cd elastix
    npm install
    ```
4.  **Backend (Optional but recommended)**: If you are working on data fetching or simulations, make sure to have the `elastix-api` running via Docker.
...
2.  Open a Pull Request against the `main` branch of `elastix`.
